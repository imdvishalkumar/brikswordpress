					...
					if ( strstr( $raw_term, '>' ) ) {
						...
						foreach ( $raw_term as $term ) {
							...
							if ( isset( $this->inserted_terms[ $taxonomy ][ $parent ][ $term ] ) ) {
								...
								$WC_CSV_Product_Import->log->add( 'CSV-Import', sprintf( __( '> > (' . __LINE__ . ') Term %s (%s) exists? %s', 'wc_csv_import' ), sanitize_text_field( $term ), esc_html( $taxonomy ), $term_may_exist ? print_r( $term_may_exist, true ) : '-' ) );
								...
								if ( ! $term_id ) {
									...
									if ( ! is_wp_error( $t ) ) {
										...
										$WC_CSV_Product_Import->log->add( 'CSV-Import', sprintf( __( '> > (' . __LINE__ . ') Failed to import term %s, parent %s - %s', 'wc_csv_import' ), sanitize_text_field( $term ), sanitize_text_field( $parent ), sanitize_text_field( $taxonomy ) ) );
										break;
									}
								}
