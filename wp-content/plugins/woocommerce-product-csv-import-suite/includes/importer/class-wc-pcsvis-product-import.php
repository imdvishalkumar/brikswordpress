...
class WC_PCSVIS_Product_Import extends WP_Importer {
	...
	public function process_product( $post ) {
		...
		if ( ! empty( $processing_product_id ) && isset( $this->processed_posts[ $processing_product_id ] ) ) {
			$this->add_import_result( 'skipped', __( 'Product already processed', 'wc_csv_import' ), $processing_product_id, $processing_product_title, $processing_product_sku );
			$this->log->add( 'csv-import', __('> Post ID already processed. Skipping.', 'wc_csv_import'), true );
			unset( $post );
			return;
		}

		if ( ! empty ( $post['post_status'] ) && $post['post_status'] == 'auto-draft' ) {
			$this->add_import_result( 'skipped', __( 'Skipping auto-draft', 'wc_csv_import' ), $processing_product_id, $processing_product_title, $processing_product_sku );
			$this->log->add( 'csv-import', __('> Skipping auto-draft.', 'wc_csv_import'), true );
			unset( $post );
			return;
		}
		...
		if ( ! $merging ) {
			if ( $this->product_exists( $processing_product_title, $processing_product_sku, $post['post_name'] ) ) {
				$this->add_import_result( 'skipped', __( 'Product already exists', 'wc_csv_import' ), $processing_product_id, $processing_product_title, $processing_product_sku );
				$this->log->add( 'csv-import', sprintf( __('> &#8220;%s&#8221; already exists.', 'wc_csv_import'), esc_html($processing_product_title) ), true );
				unset( $post );
				return;
			}
			if ( $processing_product_id && is_string( get_post_status( $processing_product_id ) ) ) {
				$this->add_import_result( 'skipped', __( 'Importing post ID conflicts with an existing post ID', 'wc_csv_import' ), $processing_product_id, get_the_title( $processing_product_id ), '' );
				$this->log->add( 'csv-import', sprintf( __('> &#8220;%s&#8221; ID already exists.', 'wc_csv_import'), esc_html( $processing_product_id ) ), true );
				unset( $post );
				return;
			}
		}
		...
		if ( $merging && $processing_product_id && get_post_type( $processing_product_id ) !== 'product' ) {
			$this->add_import_result( 'skipped', __( 'Post is not a product', 'wc_csv_import' ), $processing_product_id, $processing_product_title, $processing_product_sku );
			$this->log->add( 'csv-import', sprintf( __('> &#8220;%s&#8221; is not a product.', 'wc_csv_import'), esc_html($processing_product_id) ), true );
			unset( $post );
			return;
		}
		...
		if ( $merging ) {
			...
			$this->log->add( 'csv-import', sprintf( __('> Merging post ID %s.', 'wc_csv_import'), $post_id ), true );
			...
			if ( sizeof( $postdata ) > 1 ) {
				...
				if ( ! $result ) {
					$this->add_import_result( 'failed', __( 'Failed to update product', 'wc_csv_import' ), $post_id, $processing_product_title, $processing_product_sku );
					$this->log->add( 'csv-import', sprintf( __('> Failed to update product %s', 'wc_csv_import'), $post_id ), true );
					unset( $post );
					return;
				} else {
			...
			// Insert product
			$this->log->add( 'csv-import', sprintf( __('> Inserting %s', 'wc_csv_import'), esc_html( $processing_product_title ) ), true );
			...
			if ( is_wp_error( $post_id ) ) {
				...
				$this->add_import_result( 'failed', __( 'Failed to import product', 'wc_csv_import' ), $processing_product_id, $processing_product_title, $processing_product_sku );
				$this->log->add( 'csv-import', sprintf( __( 'Failed to import product &#8220;%s&#8221;', 'wc_csv_import' ), esc_html($processing_product_title) ) );
				unset( $post );
				return;
		...
		if ( ! empty( $post['images'] ) && is_array($post['images']) ) {
			...
			if ( $post['images'] ) foreach ( $post['images'] as $image_key => $image ) {
				...
				$this->log->add( 'csv-import', sprintf( __( '> > Importing image "%s"', 'wc_csv_import' ), $image ) );
				...
				if ( ! is_wp_error( $attachment_id ) && $attachment_id ) {
					...
					$this->log->add( 'csv-import', sprintf( __( '> > Imported image "%s"', 'wc_csv_import' ), $image ) );
					...
					$this->log->add( 'csv-import', sprintf( __( '> > Error importing image "%s"', 'wc_csv_import' ), $image ) );
			...
			$this->log->add( 'csv-import', __( '> > Images set', 'wc_csv_import' ) );
		...
		if ( $merging ) {
			...
			$this->log->add( 'csv-import', sprintf( __('> Finished merging post ID %s.', 'wc_csv_import'), $post_id ) );
		} else {
			$this->add_import_result( 'imported', 'Import successful', $post_id, $processing_product_title, $processing_product_sku );
			$this->log->add( 'csv-import', sprintf( __('> Finished importing post ID %s.', 'wc_csv_import'), $post_id ) );
		}
