...
function banner(){
    ...
    <?
	/* Added for merger on 6/2/2023 by drewadesigns.com */
    if (1 == 2 /*'67.11.197.55' != $_SERVER['REMOTE_ADDR']*/) { ?>
        <section class="banner img-fill clear" style="background-color:#E5E5E7;">
        <div class="home-page-slider-inactive">
			<div class="home-video-iframe"><iframe src="https://player.vimeo.com/video/832354393?h=c35c186e43&amp;badge=0&amp;background=1&amp;autoplay=1&amp;autopause=0&amp;player_id=0&amp;app_id=58479" frameborder="0" allow="autoplay; fullscreen; picture-in-picture" allowfullscreen title="We are Bricks R Us"></iframe></div><script src="https://player.vimeo.com/api/player.js"></script>
        </div>
        </section>
    <? } else { ?> 
    
      	
        <section class="banner img-fill clear" style="background-image:url('/wp-content/uploads/2023/12/banner-bg-2024.jpg');">
...
function latestposts($numberofposts) {
    ...
    if ($latestposts->have_posts()) {
        while ($latestposts->have_posts()) {
            ...
            // Get the featured image. Ensure $featuredimg is an array before trying to access its elements.
            $featuredimg = wp_get_attachment_image_src(get_post_thumbnail_id(get_the_id()), 'medium');
            if (!$featuredimg || !is_array($featuredimg)) {
                // Fallback image URL or logic if no featured image is found.
                $featuredimg_url = 'path/to/fallback/image.jpg';
                $width = 100; // Fallback width
                $height = 100; // Fallback height
                $bg_color = array(255, 255, 255, 1); // Fallback background color
            } else {
                $featuredimg_url = $featuredimg[0];
                $width = $featuredimg[1] ?: 1; // Ensure width is not null or 0, default to 1
                $height = $featuredimg[2] ?: 1; // Ensure height is not null or 0, default to 1
                $bg_color = array(255, 255, 255, 1); // You might want to dynamically set this based on your theme or preferences
            }
            ...
            <article id="post-<?php the_ID(); ?>" <?php post_class('col-3 col blog-style'); ?>>
                <div class="thumbnail img-fill lazy"
                     data-original="<?php echo esc_url($featuredimg_url); ?>"
                     style="background-image:url('<?php echo esc_url(create_placeholder_image($width, $height, $bg_color)); ?>');">
    ...
    wp_reset_postdata(); // Reset post data properly after custom query
