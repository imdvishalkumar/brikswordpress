...
function html5blank_header_scripts()
{
    if ($GLOBALS['pagenow'] != 'wp-login.php' && !is_admin()) {
        ...
        wp_register_script('html5blankscripts', get_template_directory_uri() . '/js/scripts.min.js#asyncload', array('jquery'), filemtime(get_template_directory().'/js/scripts.min.js')); // Custom scripts
        wp_enqueue_script('html5blankscripts'); // Enqueue it!

        wp_register_script('lazyload', get_template_directory_uri() . '/js/lib/jquery.lazyload.min.js', array('jquery'), '1.0.0'); // LazyLoad

        wp_register_script('validate', '//ajax.aspnetcdn.com/ajax/jquery.validate/1.14.0/jquery.validate.min.js#asyncload', array('jquery'), '1.14'); //VALIDATION

        wp_register_script('slick', get_template_directory_uri() . '/js/lib/slick/slick.min.js', array('jquery'), '1.5.9'); // Placeholder
        wp_register_style('slick-css', get_template_directory_uri() . '/js/lib/slick/slick.css', array(), '1.5.9', 'all');

        wp_register_script('placeholders', get_template_directory_uri() . '/js/lib/placeholders.min.js', array('jquery'), '1.0.0'); // Placeholder
        wp_enqueue_script('placeholders'); // Enqueue it!

        wp_register_script('recaptcha', 'https://www.google.com/recaptcha/api.js?onload=onloadCallback&render=explicit#asyncload', array('jquery'), '1.0'); //Recaptcha

        wp_register_script('prettyPhotonew', get_template_directory_uri() . '/js/lib/jquery.prettyPhoto.min.js', array('jquery'), '1.0.0');
        wp_register_script('prettyPhotonew-init', get_template_directory_uri() . '/js/lib/jquery.prettyPhoto.init.min.js', array('jquery'), '1.0.0');
    }
}
...
function html5blank_conditional_scripts()
{
    if(is_front_page() || is_page(132) || is_page(547)){
        wp_register_script('colorbox', get_template_directory_uri() . '/js/lib/jquery.colorbox-min.js', array('jquery'), '1.0.0'); // Colorbox
        wp_enqueue_script('colorbox'); // Enqueue it!

        wp_register_script('masonry', get_template_directory_uri() . '/js/lib/masonry.pkgd.min.js', array('jquery'), '1.0.0'); // Masonry
...
add_action('wp_enqueue_scripts','prettyPhoto',100);
...
function banner(){
    $count = count(get_field('banner_slider'));
    $slider = false;
...
function latestposts($numberofposts){
    ...
    if ($latestposts->have_posts()): while ($latestposts->have_posts()) : $latestposts->the_post();
        ...
        $featuredimg = wp_get_attachment_image_src(get_post_thumbnail_id(get_the_id()),'medium')[0];  ?>
...
function output_donors_page($atts) {
    ...
    $result  = $mysql->query( "SELECT OrganizationName, DonorSiteAddress FROM donorsiterequests WHERE OrganizationName IS NOT NULL AND OrganizationName != '' AND HideFromDonatePage = 'N' ORDER BY OrganizationName" );
    ...
    <table class="donor-list clear">
        ...
        <tbody>
            <?php foreach($sites as $site){
                ...
                $url      = 'http://www.bricksrus.com/order/' . $linkname;
                echo "<tr>
                        <td><span class='name'>$name</span></td>
                        <td><a target='_blank' href='$url'>$linkname</a></td>
                    </tr>";
            }?>
        </tbody>
    </table>
 ...
 */
function create_placeholder_image( $width = 0, $height = 0, $color = false ) {
